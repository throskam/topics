...
module.exports.sockets = {
	...
	onDisconnect: function(session, socket) {
		// if it's the last socket of the user, disconnect of all remaining chats
		if (session.user && Socket.userToSockets(session.user.id).length == 0) {
			Participant.findByUser(session.user.id).done(function (err, participants) {
